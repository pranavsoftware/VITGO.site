...
// Fetch and display plans from Firestore with future dates at the top
async function loadPlans() {
    ...
    snapshot.forEach((doc) => {
        const plan = doc.data();
        const planDateTime = new Date(plan.dateTime);  // Parse `dateTime` from each plan

        if (planDateTime > currentDateTime) {  // Only include future plans
            plans.push({ id: doc.id, ...plan });
        }
    });

    // Sort plans by date and time in ascending order (latest at the top)
    plans.sort((a, b) => new Date(a.dateTime) - new Date(b.dateTime));

    // Clear and display plans
    ...
    plans.forEach((plan) => {
        ...
        planCard.innerHTML = `
            ...
            <div class="buttons">
                ...
                <button class="btn btn-delete" onclick="deletePlan('${plan.id}', '${plan.username}')">Delete</button>
            </div>
        `;
